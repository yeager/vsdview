...
def _avoid_text_collisions(text_elements: list[str]) -> list[str]:
    """Add white background behind text and shift overlapping labels.

    Improves readability even when labels overlap by adding
    semi-transparent white backgrounds behind text.
    """
    import re as _re
    ...
    for elem in text_elements:
        ...
        # Collision detection: shift down if overlapping
        max_attempts = 4
        for _ in range(max_attempts):
            collision = False
            for (px, py, pw, ph) in placed_boxes:
                if (box_x < px + pw and box_x + est_w + 2 > px and
                        box_y < py + ph and box_y + est_h > py):
                    collision = True
                    ty += est_h + 1
                    box_y = ty - est_h * 0.55
                    break
        ...
        # but only for dark text (not light text on dark shape backgrounds)
        text_fill = ""
        _fill_m = _re.search(r'fill="([^"]+)"', elem)
        if _fill_m:
            text_fill = _fill_m.group(1).strip().upper()
        # Determine if text is "light" colored (would be on dark bg)
...
def _shapes_to_svg(shapes: list[dict], page_w: float, page_h: float,
    ...
    if all_shapes:
        ...
        if min_x < float('inf'):
            ...
            pad_x = max(20, content_w * 0.04)
            pad_y = max(20, content_h * 0.04)
